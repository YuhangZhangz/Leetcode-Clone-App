...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem, _solved }) => {
  ...
  const returnUserDataProblemData = async (transaction: Transaction) => {
  ...
  const handleLike = async () => {
    if (!user) {
      toast.error("You must be logged in to like the problem", { position: "top-left", theme: "dark" });
      return;
    }
    if (updating) return;
    setUpdating(true);
  ...
  const handleDislike = async () => {
    if (!user) {
      toast.error("You must be logged in to dislike the problem", { position: "top-left", theme: "dark" });
      return;
    }
    if (updating) return;
    setUpdating(true);
            ...
            <div className="mt-4">
              {problem.examples.map((example, index) => (
                <div key={example.id}>
                  ...
                  {example.img && (
                    <Image src={example.img} alt={`Example ${index + 1}`} width={600} height={300} className="mt-3" />
                  )}
      ...
      {youtubePlayer.isOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="relative bg-white p-4 rounded-lg max-w-2xl w-full">
            <IoClose fontSize={24} className="absolute top-2 right-2 cursor-pointer" onClick={closeModal} />
  ...
  useEffect(() => {
    const getCurrentProblem = async () => {
      ...
      if (docSnap.exists()) {
        ...
        setCurrentProblem({ id: docSnap.id, ...data } as DBProblem);
        setProblemDifficultyClass(
          data.difficulty === 'Easy' ? "bg-teal-700 text-teal-300" :
          data.difficulty === 'Medium' ? "bg-yellow-700 text-amber-400" :
  ...
  useEffect(() => {
    const getUsersDataOnProblem = async () => {
      ...
      if (userSnap.exists()) {
        ...
        setData({
          liked: d.likeProblem.includes(problemId),
          disliked: d.dislikeProblem.includes(problemId),
          starred: d.starredProblem.includes(problemId),
          solved: d.solvedProblem.includes(problemId),
        });
      }
    };
